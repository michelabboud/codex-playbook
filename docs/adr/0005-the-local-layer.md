...
3. **Three kinds of entry — Fill, Add, Override — exactly as ADR 0004 defines them**, with `L`-numbered local sections the playbook never uses and a **Dead words:** line on every Override. The grammar of that line is the same, byte for byte, so one user can carry entries between the two editions. **Completed before publication:** the sibling's first implementation refused eight of ten real entries — quoted words may contain ` · ` and parentheses, so the parser reads code spans rather than splitting on the separator; one trailing period is allowed; the bare marker anywhere but the start of a line is an error, never a skipped entry; a fenced code block is ignored. Both editions carry `tests/fixtures/dead-words-vectors.tsv`, byte-identical, and run every vector (47 at publication); file names inside the parentheses are joined by `, `, ` and ` or `, and `; a Dead-words line longer than 4,096 bytes is an error in both editions. A file named in an item is `AGENTS.md`, or a path under the skills root such as `codex-playbook-reviews/SKILL.md`.
4. **`scripts/check-local.sh` checks staleness, and the installer runs it in source preflight** — before `umask`, before any directory is created, before any backup — against the repository's own text, which is the text about to be installed. A stale override, an unparsable line or a named file that does not exist **refuses the install**, with the entry's `file:line` and the words. **Completed before publication, from the sibling's mechanical review:** an Override with no valid Dead-words line before the next entry or heading is refused too — a mistyped marker must not exit 0 — and so is a local file that exists and cannot be read. There is no flag to install past it: the fix is to re-read the rule and rewrite the entry, which is a one-line edit to a file the user owns.
